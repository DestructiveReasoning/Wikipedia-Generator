...
URLDIR = os.path.join(DATADIR, "url_lists")
all_train_urls = os.path.join(URLDIR, "all_train.txt")
all_val_urls = os.path.join(URLDIR, "all_val.txt")
all_test_urls = os.path.join(URLDIR, "all_test.txt")

TOKENDIR = os.path.join(DATADIR, "tokenized")
cnn_tokenized_stories_dir = os.path.join(TOKENDIR, "cnn")
dm_tokenized_stories_dir = os.path.join(TOKENDIR, "dailymail")

finished_files_dir = os.path.join(DATADIR, "processed")
...
def tokenize_stories(stories_dir, tokenized_stories_dir):
    ...
    with open("mapping.txt", "w") as f:
        for s in stories:
            f.write(
                "%s \t %s\n" %
                (os.path.join(
                    stories_dir, s), os.path.join(
                    tokenized_stories_dir, s)))
    command = [
        'java',
    ...
    if num_orig != num_tokenized:
        raise Exception(
            "The tokenized stories directory %s contains %i files, but it \
             should contain the same number as %s (which has %i files). Was \
             there an error during tokenization?" %
...
def get_art_abs(story_file):
    """Gets the abstract of an article"""
    lines = read_text_file(story_file)

    # Lowercase everything
    lines = [line.lower() for line in lines]

    # Put periods on the ends of lines that are missing them (this is a
    # problem in the dataset because many image captions don't end in periods;
...
def write_to_bin(url_file, out_file, makevocab=False):
    """Reads the tokenized .story files corresponding to the urls listed in
    the url_file and writes them to a out_file."""
    ...
    with open(out_file, 'w') as writer:
        for idx, s in enumerate(story_fnames):
            ...
            if os.path.isfile(os.path.join(cnn_tokenized_stories_dir, s)):
                ...
                print("Error: Couldn't find tokenized story file %s in either \
                       tokenized story directories %s and %s. Was there an \
                       error during tokenization?" % (
                    s, cnn_tokenized_stories_dir, dm_tokenized_stories_dir))
                # Check again if tokenized stories directories contain correct
                # number of files
                print("Checking that the tokenized stories directories %s and \
                       %s contain correct number of files..." % (
                ...
                raise Exception(
                    "Tokenized stories directories %s and %s contain correct \
                     number of files but story file %s found in neither." %
